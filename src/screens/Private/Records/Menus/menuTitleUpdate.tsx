...
const MenuTitleUpdate = (props: MenuTitleUpdateProps) => {
  ...
  const { user, currentGroupId, item, itemName, setItemName} = props;
  const currentItemName = item.name

  const updateTitle = () => {
    db.collection('users').doc(user.uid).collection('events').where('name', '==', currentItemName)
    .get()
    .then(function(querySnapshot) {
      ...
      db.collectionGroup('menus').where('uid', '==', user.uid).where('name', '==', currentItemName )
      .get()
      .then(function(querySnapshot) {
