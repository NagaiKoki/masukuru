...
const MenuAddModal = (props: MenuAddModalProps) => {  
  ...
  const { user, item, currentUserMenuLength, setMessageVisible, setList, isVisible, setIsVisible } = props;
  ...
  const onSubmitMenu = async () => {
    ...
    try {
      await db.collection('users').doc(user.uid).collection('events').where('name', '==', item.name).get().then(snapshot => {
